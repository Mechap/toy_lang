/*
        ...
        Otherwise, it should support most of the "typical use-case" features that std::format
   provides. In essence it is a type-safe alternative to printf supporting custom type printers. The
   usage is as such:

        zpr::println("{<spec>}", argument);

        where `<spec>` is exactly a `printf`-style format specifier (note: there is no leading colon
   unlike the fmtlib/python style), and where the final type specifier (eg. `s`, `d`) is optional.
   Floating point values will print as if `g` was used. Size specifiers (eg. `lld`) are not
   supported. Variable width and precision specifiers (eg. `%.*s`) are not supported.
        ...
        For non-constant widths and precisions, there are 3 functions: w(int), p(int), and wp(int,
   int). these functions return a function object, which you should then call with the actual
   argument value. usage:

        old: zpr::println("{.*}", 3, M_PI);
        new: zpr::println("{}", zpr::p(3)(M_PI));

        for wp(), the width is the first argument, and the precision the second argument. Note that
   you could use this everywhere and not put the widths in the format string at all.

        There are optional #define macros to control behaviour; defining them without a value
   constitues a TRUE -- for FALSE, either `#undef` them, or explicitly define them as 0. For values
   that are TRUE by default, you *must* explicitly `#define` them as 0.

        - ZPR_USE_STD
                this is *TRUE* by default. controls whether or not STL type interfaces are used;
   with it, you get the std::pair printer, and the sprint() overload that returns std::string.
   that's about it. iterator-based container printing is not affected by this flag.

        - ZPR_HEX_0X_RESPECTS_UPPERCASE
                this is *FALSE* by default. basically, if you use '{X}', this setting determines
   whether you'll get '0xDEADBEEF' or '0XDEADBEEF'. i think the capital 'X' looks ugly as heck, so
   this is OFF by default.

        - ZPR_DECIMAL_LOOKUP_TABLE
                this is *TRUE* by default. controls whether we use a lookup table to increase the
   speed of decimal printing. this uses 201 bytes.

        - ZPR_HEXADECIMAL_LOOKUP_TABLE
                this is *TRUE* by default. controls whether we use a lookup table to increase the
   speed of hex printing. this uses 1025 bytes.

        - ZPR_FREESTANDING
                this is *FALSE by default; controls whether or not a standard library implementation
   is available. if not, then the following changes are made: (a) the print() family of functions
   that would print to stdout, and the fprint() family that would print to a FILE* are no longer
   available.

                (b) memset(), memcpy(), memmove(), strlen(), and strncpy() are forward declared
   according to the C library specifications, but they are not defined. they are expected to be
   defined elsewhere in the program.
        ...
        To format custom types, specialise the print_formatter struct. An example of how it should
   be done can be seen from the builtin formatters, taking note to follow the signatures.

        A key point to note is that you can specialise both for the 'raw' type -- ie. you can
   specialise for T&, and also for T; the non-decayed specialisation is preferred if both exist.

        (NB: the reason for this is so that we can support reference to array of char, ie. char
   (&)[N])
        ...
        * only available if ZPR_FREESTANDING != 0: print to the specified FILE*, followed by a
   newline '\n'. size_t fprintln(FILE* file, tt::str_view fmt, Args&&... args);
        ...
        - change the signature of zpr::sprint(char* buf, size_t sz, str_view fmt, ...);
          ...
          this is necessary because msvc cannot differentiate between sprint(char*, size_t,
   str_view, ...) and sprint(str_view, ...), and would complain about an ambiguous overload.
        ...
        2.2.1 - 01/05/2021
        ------------------
        Improve the `file_appender` so that newlines are written together with the last part of the
   buffer in a single call to fwrite() -- this hopefully mitigates lines being broken up right at
   the newline when printing in multithreaded scenarios.



        2.2.0 - 27/04/2021
        ------------------
        Add 'alternate' flag for the iterable printers; if true, then the opening and closing
   brackets ('[' and ']') and the commas (',') between items are *not* printed. As a reminder, use
   '{#}' to specify alternate printing mode.
        ...
        -------------------
        Bug fixes:
        - fix handling of control macros when they are defined without a value. Now they will not
   generate errors, and are treated as TRUE. Also update the documentation about this.
        ...
        ------------------
        Bug fixes:
        - fix an issue preventing user-defined formatters from calling cprint() with the callback
   given to them
        ...
        2.1.1 - 29/09/2020
        ------------------
        Remove tt::forward and tt::move to cut down on templates (they're just glorified
   static_cast<T&&>s anyway)
        ...
        2.1.0 - 28/09/2020
        ------------------
        Remove redundant overloads of all the print functions taking const char (&)[N], since
   tt:str_view has an implicit constructor for that.
        ...
        2.0.2 - 28/09/2020
        ------------------
        Small performance improvements. Reduced enable_if usage in favour of template specialisation
   to improve compile times.
        ...
        2.0.0 - 27/09/2020
        ------------------
        Completely rewrite the internals; now no longer tuple-based, and we're back to template
   packs. However, instead of using recursion, we are now expanding with fold expressions. This
   comes with one drawback, however; we now do not support in-stream width and precision specifiers
   (eg. {*.s}). instead, we expose 3 new functions, w(), p(), and wp(), which do what their names
   would suggest; see the documentation above for more info.
        ...
        1.6.0 - 26/09/2020
        ------------------
        Add the ZPR_FREESTANDING define; see documentation above for details. Also add cprint() and
   family, which allow using a callback of the form `void (*)(char*, size_t)` -- or a compatible
   template function object, to print.
        ...
        1.5.0 - 10/09/2020
        ------------------
        Change the print_formatter interface subtly. Now, you can choose to specialise the template
   for both the decayed type (eg. int, const char*), or for the un-decayed type (eg. const int&,
   const char (&)[N]). The compiler will choose the appropriate specialisation, preferring the
   latter (not-decayed) type, if it exists.

        Bug fixes:
        - fix issues where we would write NULL bytes into the output stream, due to the 'char(&)[N]'
   change.
        ...
        1.4.0 - 10/09/2020
        ------------------
        Completely remove dependency on STL types. sadly this includes lots of re-implemented
   type_traits, but an okay cost to pay, I suppose. Introduces ZPR_USE_STD define to control this.
        ...
        1.3.0 - 08/09/2020
        ------------------
        Add overloads for the user-facing print functions that accept std::string_view as the format
   string. This also works for std::string since string_view has an implicit conversion constructor
   for it.

        Removed user-facing overloads of print and friends that take 'const char*'; now they either
   take std::string_view (as above), or (const char&)[].

        Change the behaviour of string printers; now, any iterable type with a value_type typedef
   equal to 'char' (exactly char -- not signed char, not unsigned char) will print as a string. This
   lets us cover custom string types as well. A side effect of this is that std::vector<char> will
   print as a string, which might be unexpected.
        ...
        1.2.0 - 20/07/2020
        ------------------
        Use floating-point printer from mpaland/printf, letting us actually beat printf in all
   cases.
        ...
        1.1.0 - 20/07/2020
        ------------------
        Performance improvements: switched to tuple-based arguments instead of parameter-packed
   recursion. We are now (slightly) faster than printf (as least on two of my systems) as long as no
   floating-point is involved. (for now we are still forced to call snprintf to print floats...
   charconv pls)
...
namespace zpr::tt {
    ...
    template <>
    struct is_integral_base<signed short> : true_type {};
    template <>
    struct is_integral_base<signed int> : true_type {};
    template <>
    struct is_integral_base<signed long> : true_type {};
    template <>
    struct is_integral_base<signed long long> : true_type {};
    template <>
    ...
    template <>
    struct is_signed_base<signed short> : true_type {};
    template <>
    struct is_signed_base<signed int> : true_type {};
    template <>
    struct is_signed_base<signed long> : true_type {};
    template <>
    struct is_signed_base<signed long long> : true_type {};

    template <typename T>
    struct is_signed : is_signed_base<remove_cv_t<T>> {};
    template <typename T>
    ...
    struct str_view {
        ...
        inline bool operator!=(const str_view& other) const { return !(*this == other); }
        ...
        inline str_view drop(size_t n) const {
            return (this->size() >= n ? this->substr(n, this->size() - n) : "");
        }
        inline str_view take(size_t n) const {
            return (this->size() >= n ? this->substr(0, n) : *this);
        }
        inline str_view take_last(size_t n) const {
            return (this->size() >= n ? this->substr(this->size() - n, n) : *this);
        }
        inline str_view drop_last(size_t n) const {
      ...
      private:
        inline str_view substr(size_t pos, size_t cnt) const {
            return str_view(this->ptr + pos, cnt);
        }
...
namespace zpr {
    ...
    struct format_args {
        ...
        bool negative_width() const {
            return have_width() && (this->flags & FMT_FLAG_WIDTH_NEGATIVE);
        }
        ...
        void set_width(int64_t w) {
            ...
            if (w < 0) this->flags |= FMT_FLAG_WIDTH_NEGATIVE;
        }
    };
    ...
    namespace detail {
        ...
        struct __fmtarg_wp {
            ...
            __fmtarg_wp(T&& x, int width, int prec)
                : arg(static_cast<T&&>(x)), prec(prec), width(width) {}
        ...
        struct __fmtarg_w_helper {
            ...
            template <typename T>
            inline __fmtarg_w<T&&> operator()(T&& val) {
        ...
        struct __fmtarg_p_helper {
            ...
            template <typename T>
            inline __fmtarg_p<T&&> operator()(T&& val) {
        ...
        struct __fmtarg_wp_helper {
            ...
            template <typename T>
            inline __fmtarg_wp<T&&> operator()(T&& val) {
        ...
        struct has_formatter<
            T, tt::void_t<decltype(tt::declval<print_formatter<T>>().print(
                   tt::declval<T>(), dummy_appender(), {}))>> : tt::true_type {};
        ...
        template <typename T>
        struct is_iterable<
            T, tt::void_t<decltype(begin(tt::declval<T&>())), decltype(end(tt::declval<T&>()))>>
            : tt::true_type {};
        ...
        static inline format_args parse_fmt_spec(tt::str_view sv) {
            ...
            format_args fmt_args = {};
            {
                while (sv.size() > 0) {
                    switch (sv[0]) {
                        case '0':
                            fmt_args.flags |= FMT_FLAG_ZERO_PAD;
                            sv.remove_prefix(1);
                            continue;
                        case '#':
                            fmt_args.flags |= FMT_FLAG_ALTERNATE;
                            sv.remove_prefix(1);
                            continue;
                        case '-':
                            fmt_args.flags |= FMT_FLAG_WIDTH_NEGATIVE;
                            sv.remove_prefix(1);
                            continue;
                        case '+':
                            fmt_args.flags |= FMT_FLAG_PREPEND_PLUS;
                            sv.remove_prefix(1);
                            continue;
                        case ' ':
                            fmt_args.flags |= FMT_FLAG_PREPEND_SPACE;
                            sv.remove_prefix(1);
                            continue;
                        default:
                            break;
                ...
                if (sv.size() >= 2 && sv[0] == '.') {
                    ...
                    if (sv[0] == '-') {
                        ...
                        size_t k = 1;
                        while (sv.size() > k && ('0' <= sv[k] && sv[k] <= '9')) k++;
                ...
                if (!sv.empty()) fmt_args.specifier = sv[0];
            }
        ...
        size_t print_string(CallbackFn&& cb, const char* str, size_t len, format_args args) {
            ...
            if (args.have_precision())
                string_length = tt::min(args.precision, static_cast<int64_t>(len));
            else
                string_length = static_cast<int64_t>(len);
        ...
        size_t print_special_floating(CallbackFn&& cb, double value, format_args args) {
            ...
            if (value != value) return print_string(cb, "nan", 3, static_cast<format_args&&>(args));
            ...
            if (value > DBL_MAX) {
                return print_string(
                    cb,
                    args.prepend_plus()    ? "+inf"
                    : args.prepend_space() ? " inf"
                                           : "inf",
        ...
        size_t print_exponent(CallbackFn&& cb, double value, format_args args) {
            ...
            int prec =
                (args.have_precision() ? static_cast<int>(args.precision) : DEFAULT_PRECISION);
            ...
            auto expval = static_cast<int64_t>(
                0.1760912590558 + exp2 * 0.301029995663981 + (conv.F - 1.5) * 0.289529654602168);
            ...
            // the exponent format is "%+02d" and largest value is "307", so set aside 4-5
            // characters (including the e+ part)
            ...
            if (args.specifier == 'g' || args.specifier == 'G') {
                ...
                if ((value >= 1e-4) && (value < 1e6)) {
                    ...
                    if (prec > 0 && use_precision) prec -= 1;
                }
            }
            ...
            auto len =
                static_cast<int64_t>(print_floating(cb, negative ? -value : value, args_copy));

            // output the exponent part
            if (minwidth > 0) {
                len++;
                if (args.specifier & 0x20)
                    cb('e');
                else
                    cb('E');
                ...
                auto buf =
                    print_decimal_integer(digits_buf, 8, static_cast<int64_t>(tt::abs(expval)));
                ...
                if (use_right_pad && args.width > len) cb(' ', args.width - len), len = args.width;
            }
        ...
        size_t print_floating(CallbackFn&& cb, double value, format_args args) {
            ...
            // default to g.
            if (args.specifier == -1) args.specifier = 'g';
            ...
            if (prec == 0U) {
                ...
                // add extra 0s
                while ((len < MAX_BUFFER_LEN) && (count-- > 0)) buf[len++] = '0';

                // add decimal
                if (len < MAX_BUFFER_LEN) buf[len++] = '.';
            }
            ...
            if (use_zero_pad) {
                ...
                if (args.have_width() != 0 &&
                    (negative || args.prepend_plus() || args.prepend_space()))
                    width--;
            ...
            // reverse it.
            for (size_t i = 0; i < len / 2; i++) tt::swap(buf[i], buf[len - i - 1]);
        ...
        char* print_hex_integer(char* buf, size_t bufsz, T value) {
            ...
            constexpr auto copy = [](char* dst, const char* src) { memcpy(dst, src, 2); };
        ...
        char* print_integer(char* buf, size_t bufsz, T value, int base) {
            if (base == 2)
                return print_binary_integer(buf, bufsz, value);
            else if (base == 16)
                return print_hex_integer(buf, bufsz, value);
            else
                return print_decimal_integer(buf, bufsz, value);
        }
        ...
        void print_one(CallbackFn&& cb, format_args fmt_args, T&& value) {
            ...
            static_assert(
                has_formatter_v<T> || has_formatter_v<Decayed_T>, "no formatter for type");

            if constexpr (has_formatter<T>::value) {
                print_formatter<T>().print(
                    static_cast<T&&>(value), static_cast<CallbackFn&&>(cb),
                    static_cast<format_args&&>(fmt_args));
            } else {
                print_formatter<Decayed_T>().print(
                    static_cast<T&&>(value), static_cast<CallbackFn&&>(cb),
                    static_cast<format_args&&>(fmt_args));
            }
        }
        ...
        void skip_fmts(__print_state_t* pst, CallbackFn&& cb, T&& value) {
            while ((static_cast<size_t>(pst->end - pst->fmt) <= pst->len) && pst->end &&
                   *pst->end) {
                if (*pst->end == '{') {
                    auto tmp = pst->end;
                    ...
                    while (pst->end[0] && pst->end[0] != '}') pst->end++;
                    ...
                    print_one(
                        static_cast<CallbackFn&&>(cb), static_cast<format_args&&>(fmt_spec),
                        static_cast<T&&>(value));
                    ...
                    // well... we don't need to escape }, but for consistency, we accept either } or
                    // }} to print one }.
                    if (pst->end[1] == '}') pst->end++;
        ...
        struct string_appender {
            ...
            inline void operator()(tt::str_view sv) {
                this->buf += std::string_view(sv.data(), sv.size());
            }
            ...
            inline void operator()(const char* begin, const char* end) {
                this->buf.append(begin, end);
            }
            inline void operator()(const char* begin, size_t len) {
                this->buf.append(begin, begin + len);
            }
        ...
        struct file_appender {
            ...
            inline void operator()(const char* begin, const char* end) {
                (*this)(begin, static_cast<size_t>(end - begin));
            }
          ...
          private:
            inline size_t remaining() { return Limit - (ptr - buf); }

            inline void flush(bool last = false) {
                if (!last && static_cast<size_t>(ptr - buf) < Limit) {
                    if constexpr (Newline) this->buf[ptr - buf] = '\n';
        ...
        struct callback_appender {
            callback_appender(Fn* callback, bool newline)
                : len(0), newline(newline), callback(callback) {}
            ~callback_appender() {
                if (newline) {
                    (*callback)("\n", 1);
                }
            ...
            inline void operator()(char c, size_t n) {
                for (size_t i = 0; i < n; i++) (*callback)(&c, 1);
        ...
        struct buffer_appender {
            ...
            inline void operator()(char c) {
                if (this->len < this->cap) this->buf[this->len++] = c;
            }
            ...
            inline void operator()(char c, size_t n) {
                for (size_t i = 0; i < this->remaining(n); i++) this->buf[this->len++] = c;
            }
...
// if we are freestanding, we don't have stdio, so we can't print to "stdout".
    ...
    size_t print(tt::str_view fmt, Args&&... args) {
        size_t ret = 0;
        detail::print(
            detail::file_appender<detail::STDIO_BUFFER_SIZE, false>(stdout, ret), fmt,
    ...
    size_t println(tt::str_view fmt, Args&&... args) {
        size_t ret = 0;
        detail::print(
            detail::file_appender<detail::STDIO_BUFFER_SIZE, true>(stdout, ret), fmt,
            static_cast<Args&&>(args)...);
    ...
    size_t fprint(FILE* file, tt::str_view fmt, Args&&... args) {
        size_t ret = 0;
        detail::print(
            detail::file_appender<detail::STDIO_BUFFER_SIZE, false>(file, ret), fmt,
            static_cast<Args&&>(args)...);
    ...
    size_t fprintln(FILE* file, tt::str_view fmt, Args&&... args) {
        size_t ret = 0;
        detail::print(
            detail::file_appender<detail::STDIO_BUFFER_SIZE, true>(file, ret), fmt,
            static_cast<Args&&>(args)...);
    ...
    inline detail::__fmtarg_w_helper w(int width) { return detail::__fmtarg_w_helper(width); }

    inline detail::__fmtarg_p_helper p(int prec) { return detail::__fmtarg_p_helper(prec); }
    ...
    struct print_formatter<detail::__fmtarg_w<T>> {
        ...
        void print(F&& x, Cb&& cb, format_args args) {
            ...
            detail::print_one(
                static_cast<Cb&&>(cb), static_cast<format_args&&>(args), static_cast<T&&>(x.arg));
        }
    };
    ...
    namespace detail {
        ...
        struct __int_formatter {
            ...
            void print(T x, Cb&& cb, format_args args) {
                int base = 10;
                if ((args.specifier | 0x20) == 'x')
                    base = 16;
                else if (args.specifier == 'b')
                    base = 2;
...

                {
                    if constexpr (tt::is_unsigned<Decayed_T>::value) {
                        ...
                        if (base == 16) {
                            digits = detail::print_integer(
                                digits_buf, digits_buf_sz,
                            ...
                            digits =
                                detail::print_integer(digits_buf, digits_buf_sz, abs_val, base);
                ...
                int64_t prefix_digits_length = 0;
                {
                    ...
                    if (base != 10 && args.alternate()) {
                        *pf++ = '0';
                        *pf++ =
                            (ZPR_HEX_0X_RESPECTS_UPPERCASE ? args.specifier
                                                           : (args.specifier | 0x20));
                ...
                int64_t output_length_with_precision =
                    (args.have_precision()
                         ? tt::max(args.precision, static_cast<int64_t>(digits_len))
                         : static_cast<int64_t>(digits_len));

                int64_t total_digits_length =
                    prefix_digits_length + static_cast<int64_t>(digits_len);
    ...
    template <>
    struct print_formatter<signed char> : detail::__int_formatter<signed char> {};
    template <>
    struct print_formatter<unsigned char> : detail::__int_formatter<unsigned char> {};
    template <>
    struct print_formatter<signed short> : detail::__int_formatter<signed short> {};
    template <>
    struct print_formatter<unsigned short> : detail::__int_formatter<unsigned short> {};
    template <>
    struct print_formatter<signed int> : detail::__int_formatter<signed int> {};
    template <>
    struct print_formatter<unsigned int> : detail::__int_formatter<unsigned int> {};
    template <>
    struct print_formatter<signed long> : detail::__int_formatter<signed long> {};
    template <>
    struct print_formatter<unsigned long> : detail::__int_formatter<unsigned long> {};
    template <>
    struct print_formatter<signed long long> : detail::__int_formatter<signed long long> {};
    template <>
    struct print_formatter<unsigned long long> : detail::__int_formatter<unsigned long long> {};

    template <typename T>
    struct print_formatter<T, typename tt::enable_if<(tt::is_enum_v<tt::decay_t<T>>)>::type> {
        template <typename Cb>
        void print(T x, Cb&& cb, format_args args) {
            using underlying = tt::underlying_type_t<tt::decay_t<T>>;
            detail::print_one(
                static_cast<Cb&&>(cb), static_cast<format_args&&>(args),
    ...
    struct print_formatter<const char*> {
        ...
        void print(const char* x, Cb&& cb, format_args args) {
            detail::print_string(
                static_cast<Cb&&>(cb), x, strlen(x), static_cast<format_args&&>(args));
        }
    };
    ...
    struct print_formatter<bool> {
        ...
        void print(bool x, Cb&& cb, format_args args) {
            detail::print_string(
                static_cast<Cb&&>(cb), x ? "true" : "false", x ? 4 : 5,
                static_cast<format_args&&>(args));
        }
    };
    ...
    template <typename T>
    struct print_formatter<
        T, typename tt::enable_if<(
               tt::conjunction<
                   detail::is_iterable<T>,
                   tt::is_same<tt::remove_cv_t<typename T::value_type>, char>>::value)>::type> {
        template <typename Cb>
        void print(const T& x, Cb&& cb, format_args args) {
            detail::print_string(
                static_cast<Cb&&>(cb), x.data(), x.size(), static_cast<format_args&&>(args));
        }
    };
    ...
    struct print_formatter<
        T, typename tt::enable_if<(
               tt::conjunction<
                   detail::is_iterable<T>,
                   tt::negation<tt::is_same<typename T::value_type, char>>>::value)>::type> {
        template <typename Cb>
        void print(const T& x, Cb&& cb, format_args args) {
            if (begin(x) == end(x)) {
                if (!args.alternate()) cb("[ ]");
                return;
            }
    ...
    std::string sprint(tt::str_view fmt, Args&&... args) {
        std::string buf;
        detail::print(detail::string_appender(buf), fmt, static_cast<Args&&>(args)...);
