/*
        ...
        This is a freestanding library that can be compiled without any libc or STL headers, apart
   from freestanding headers (stdint.h and stddef.h).

        A simple and lightweight library implementing 'stretchy buffers' and immutable views of
   them, akin to std::vector<T> and std::span<T>, but operating purely on the basis of bytes. The
   intended usecase for this library is dealing with data representation and exchange as byte
   arrays.

        A lightweight replacement for std::string_view is also included (str_view) that has a more
   ergonomic API, namely drop(), drop_last(), take(), and take_last() that return copies, instead of
   being limited to remove_prefix() and remove_suffix().
   ...
   with it, str_view gains implicit constructors accepting std::string and std::string_view, as well
   ...
   is available. if not, then memset(), memmove(), strlen(), and strncmp() are forward declared
...
namespace zbuf {
    ...
    struct str_view {
        ...
        template <
            typename T,
            typename = typename detail::enable_if<detail::is_same<const char*, T>::value>::type>
        ...
        inline size_t find(str_view sv) const {
            ...
            for (size_t i = 0; i < 1 + this->size() - sv.size(); i++) {
                if (this->drop(i).take(sv.size()) == sv) return i;
            }
        ...
        inline size_t rfind(str_view sv) const {
            ...
            for (size_t i = 1 + this->size() - sv.size(); i-- > 0;) {
                if (this->take(1 + i).take_last(sv.size()) == sv) return i;
            }
        ...
        inline str_view substr(size_t pos = 0, size_t cnt = -1) const {
            return str_view(
                this->ptr + pos,
                cnt == static_cast<size_t>(-1) ? (this->len > pos ? this->len - pos : 0) : cnt);
        }
    ...
    struct Buffer {
        ...
        inline Span span() const;
        inline str_view sv() const {
            return str_view(reinterpret_cast<const char*>(this->ptr), this->len);
        }
        ...
        inline void incrementSize(size_t by) { this->len += by; }
        ...
        inline void unsafeClear() { this->len = 0; }
        ...
        inline void autoWrite(const void* data, size_t len) {
            if (this->remaining() < len) this->grow(len - this->remaining());

            this->write(data, len);
        }

        // auto-expands by 1.6x
        inline void grow() { this->resize(this->cap * 1.6); }

        // expands only by the specified amount
        inline void grow(size_t sz) { this->resize(this->cap + sz); }

        // changes the size to be sz. (only expands, never contracts)
        inline void resize(size_t sz) {
            if (sz < this->cap) return;
        ...
        static inline Buffer empty() { return Buffer(0); }
    ...
    struct Span {
        ...
        inline size_t size() const { return this->len; }
        inline const uint8_t* data() const { return this->ptr; }
        inline zbuf::str_view sv() const {
            return zbuf::str_view(reinterpret_cast<const char*>(this->ptr), this->len);
        }

        inline Span& remove_prefix(size_t n) {
            n = detail::min(n, this->len);
            this->ptr += n;
            this->len -= n;
            return *this;
        }
        inline Span& remove_suffix(size_t n) {
            n = detail::min(n, this->len);
            this->len -= n;
            return *this;
        }

        inline Span drop(size_t n) const {
            auto copy = *this;
            return copy.remove_prefix(n);
        }
        inline Span take(size_t n) const {
            auto copy = *this;
            return copy.remove_suffix(this->len > n ? this->len - n : 0);
        }

        inline Span take_last(size_t n) const {
            return (this->size() >= n ? Span(this->ptr + n, this->size() - n) : *this);
        }
        inline Span drop_last(size_t n) const {
            return (this->size() >= n ? Span(this->ptr, this->size() - n) : *this);
        }
    ...
    inline Span Buffer::span() const { return Span(this->ptr, this->len); }

    inline size_t Buffer::write(Span s) { return this->write(s.data(), s.size()); }

    inline void Buffer::autoWrite(Span s) { return this->autoWrite(s.data(), s.size()); }

    inline Span str_view::span() const { return Span(this->data(), this->size()); }
}  // namespace zbuf
