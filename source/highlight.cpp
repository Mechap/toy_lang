...
namespace lc {
    ...
    static void int_highlight(
        State& st, const Expr* expr, std::string& top, std::string& bot, bool combine = false,
        ...
        if (auto v = dynamic_cast<const Var*>(expr); v) {
            ...
            int_highlight(
                st, a->arg, top, bot, /* combine: */ false,
                /* omit_lambda_parens: */ omit_lambda_parens);

            if (close) add(")", under);
        } else if (auto f = dynamic_cast<const Lambda*>(expr); f) {
            bool close = false;
            if (!combine) {
                if (!omit_lambda_parens) close = true, add("(", under);

                if (st.flags & FLAG_HASKELL_STYLE)
                    add("\\", under);
                else
                    add("λ", under);
            }
            ...
            if (st.flags & FLAG_ABBREV_LAMBDA) st.combined_args.insert(f->arg);

            bool omit_next_parens = false;
            if (auto inner = dynamic_cast<Lambda*>(f->body);
                (st.flags & FLAG_ABBREV_LAMBDA) && inner) {
            ...
            normal:
                ...
                int_highlight(
                    st, f->body, top, bot, /* combine: */ false,
    ...
    std::pair<std::string, std::string> highlight(
        const Expr* expr, std::function<std::optional<std::string>(const ast::Expr*)> pred,
        std::function<std::optional<std::string>(const ast::Expr*)> arg_pred, int flags) {
        State st{};
        st.pred = std::move(pred), st.flags = flags;
    ...
    const static auto BETA_SUB_HIGHLIGHT =
        zpr::sprint("{}{}{}", BLUE_BOLD, UNDERLINE, COLOUR_RESET);
    const static auto BETA_ARG_HIGHLIGHT =
        zpr::sprint("{}{}{}", GREEN_BOLD, UNDERLINE, COLOUR_RESET);

    std::pair<std::string, std::string> logAlphaConversion(
        const Expr** whole, const Expr* sub, int print_flags) {
        return highlight(
            *whole,
            [&](const Expr* x) -> std::optional<std::string> {
                if (x == sub)
                    return ALPHA_HIGHLIGHT;
                else
                    return std::nullopt;
            },
            [](auto) { return std::nullopt; }, print_flags);
    }

    std::pair<std::string, std::string> logBetaReduction(
        const Expr** whole, const Expr* fn, const Expr* arg, const std::vector<Expr**>& _substs,
        int print_flags) {
        std::set<const Expr*> subs;
        std::transform(
            _substs.begin(), _substs.end(), std::inserter(subs, subs.begin()),
            [](auto x) { return *x; });
        ...
        return highlight(
            *whole,
            [&](const Expr* e) -> std::optional<std::string> {
                if (e == arg)
                    return BETA_ARG_HIGHLIGHT;
                else if (subs.find(e) != subs.end())
                    return BETA_SUB_HIGHLIGHT;
                else
                    return std::nullopt;
            },
    ...
    std::string print(const ast::Expr* expr, int flags) {
        return highlight(
                   expr, [](auto) { return std::nullopt; }, [](auto) { return std::nullopt; },
                   flags)
            .first;
    }

    std::string print(
        const ast::Expr* expr, std::function<std::optional<std::string>(const ast::Expr*)> replace,
        int flags) {
        State st{};
