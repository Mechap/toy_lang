...
namespace lc {
    ...
    void evalLine(Context& ctx, zbuf::str_view sv) {
        ...
        if (!expr_or_error) return parseError(expr_or_error.error(), input);
            ...
            replaced = lc::print(
                ...
                [&](const ast::Expr* expr) -> std::optional<std::string> {
                    for (auto& [name, var] : ctx.vars) {
                        ...
                        if (alpha_equivalent(ctx, expr, var)) return name;
                    }
    ...
    void runReplCommand(Context& ctx, zbuf::str_view input) {
        auto print_thingy = [&ctx](const char* thing, int f) {
            bool en = (ctx.flags & f);
            zpr::println(
                "{}*.{} {} {}{}{}", BLACK_BOLD, COLOUR_RESET, thing, en ? GREEN_BOLD : RED_BOLD,
    ...
    void printError(zbuf::str_view msg) {
        zpr::fprintln(
            stderr, "{}error:{} {}{}{}", RED_BOLD, COLOUR_RESET, BLACK_BOLD, msg, COLOUR_RESET);
    }
    ...
    zbuf::str_view trim(zbuf::str_view s) {
        while (s.front() == ' ' || s.front() == '\t') s.remove_prefix(1);

        while (s.back() == ' ' || s.back() == '\t') s.remove_suffix(1);
