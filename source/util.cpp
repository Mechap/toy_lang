...
namespace lc {
    ...
    static std::pair<Expr*, bool> replace_vars_once(
        const Context& ctx, const std::set<const Var*>& free_vars, const Expr* expr) {
    ...
    std::vector<Expr**> find_substitutions(Expr** expr, const std::string& var) {
        if (auto v = dynamic_cast<Var*>(*expr); v != nullptr) {
            ...
            if (l->arg != var)
                return find_substitutions(&l->body, var);
            else
                return {};
        } else {
            abort();
    ...
    Lambda* substitute(Lambda* expr, const std::vector<Expr**>& vars, const Expr* value) {
        for (auto v : vars) *v = value->clone();

        return expr;
    }

    template <
        bool Bound, int MaxDepth = INT_MAX,
        typename Retty =
            std::conditional_t<Bound, std::map<std::string, Lambda*>, std::set<const Var*>>>
    static Retty _find_variables(
        std::map<std::string, Lambda*> seen, const Expr* expr, int depth = 0) {
        if (auto v = dynamic_cast<const Var*>(expr); v != nullptr) {
            if constexpr (Bound) {
                if (auto it = seen.find(v->name); it != seen.end()) return {*it};

                return {};
            } else {
                if (seen.find(v->name) == seen.end()) return {v};
    ...
    std::string fresh_name(const std::string& name) { return name + "'"; }

    template <typename Container, typename Fn>
    static auto map(Container&& c, Fn&& func)
        -> std::set<decltype(func(std::declval<typename std::decay_t<Container>::value_type>()))> {
    ...
    static bool alpha_equivalent(
        const Expr* a, const Expr* b, int cur_depth, const CheckState& sta, const CheckState& stb) {
        ...
        if (free_a_names != free_b_names) return false;
        ...
        if (auto v1 = dynamic_cast<const Var*>(a), v2 = dynamic_cast<const Var*>(b); v1 && v2) {
            ...
            return false;
        } else if (auto a1 = dynamic_cast<const Apply*>(a), a2 = dynamic_cast<const Apply*>(b);
                   a1 && a2) {
            return alpha_equivalent(a1->fn, a2->fn, cur_depth, sta, stb) &&
                   alpha_equivalent(a1->arg, a2->arg, cur_depth, sta, stb);
        } else if (auto l1 = dynamic_cast<const Lambda*>(a), l2 = dynamic_cast<const Lambda*>(b);
                   l1 && l2) {
            auto sta1 = sta;
            auto stb1 = stb;
