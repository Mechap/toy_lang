...
namespace parser {
    ...
    static size_t is_valid_identifier(zbuf::str_view str) {
        ...
        k = unicode::is_category(
            str, {UTF8PROC_CATEGORY_MN, UTF8PROC_CATEGORY_MC, UTF8PROC_CATEGORY_ME,
    ...
    static zst::Result<Token, Error> lex_one_token(zbuf::str_view& src, size_t& idx, TT prevType) {
        ...
        while (src.size() > 0 &&
               (k = unicode::is_category(
                    src, {UTF8PROC_CATEGORY_ZS, UTF8PROC_CATEGORY_ZL, UTF8PROC_CATEGORY_ZP}),
                k > 0)) {
            src.remove_prefix(k);
            idx += k;
        }

        if (src.empty()) return Ok(Token(TT::EndOfFile, Location{idx, 0}, ""));
...
#define MATCH_MULTICHAR_TOKEN(match, tok)                                       \
    (src.find(match) == 0) {                                                    \
        auto x = strlen(match);                                                 \
        ...
        if MATCH_MULTICHAR_TOKEN ("->", TT::RightArrow)
            else if MATCH_MULTICHAR_TOKEN ("λ", TT::Lambda) else if (is_valid_identifier(src) > 0) {
            ...
            switch (src[0]) {
                case '(':
                    ret = Token(TT::LParen, loc, src.take(1));
                    break;
                case ')':
                    ret = Token(TT::RParen, loc, src.take(1));
                    break;
                case '.':
                    ret = Token(TT::Period, loc, src.take(1));
                    break;
                case '$':
                    ret = Token(TT::Dollar, loc, src.take(1));
                    break;
                case '=':
                    ret = Token(TT::Equal, loc, src.take(1));
                    break;
                case '\\':
                    ret = Token(TT::Lambda, loc, src.take(1));
                    break;

                default: {
                    auto sz = unicode::get_codepoint_length(src);
                    return Err(
                        Error{zpr::sprint("invalid token '{}'", src.take(sz)), Location{idx, 1}});
                }
            }
...
namespace unicode {
    ...
    size_t is_letter(zbuf::str_view str) {
        return is_category(
            str, {UTF8PROC_CATEGORY_LU, UTF8PROC_CATEGORY_LL, UTF8PROC_CATEGORY_LT,
                  UTF8PROC_CATEGORY_LM, UTF8PROC_CATEGORY_LO});
    }

    size_t is_digit(zbuf::str_view str) { return is_category(str, {UTF8PROC_CATEGORY_ND}); }
}  // namespace unicode
