...
namespace lc {
    ...
    void loadFile(Context& ctx, zbuf::str_view path) {
        ...
        if (!lines_or_err) return printError(lines_or_err.error());
        ...
        for (size_t i = 0; i < lines.size(); i++) {
            ...
            if (!expr_or_err) {
                ...
                zpr::println(
                    "{}*.{} {}warning:{} file '{}' not loaded completely ({} line{})", BLACK_BOLD,
                    COLOUR_RESET, YELLOW_BOLD, COLOUR_RESET, path, i, i == 1 ? "" : "s");
                return;
            }
        ...
        zpr::println(
            "{}*.{} loaded {} line{} from '{}'", BLACK_BOLD, COLOUR_RESET, lines.size(),
            lines.size() == 1 ? "" : "s", path);
    }
...
static std::vector<zbuf::str_view> split_string(zbuf::str_view view) {
    ...
    // account for the case when there's no trailing newline, and we still have some stuff stuck in
    // the view.
    if (!view.empty()) ret.emplace_back(view.data(), view.length());
