...
namespace parser {
    ...
    namespace {
        ...
        struct is_result<zst::Result<T, E>> : std::true_type {};
        ...
        std::tuple<std::optional<typename A::value_type>> __unwrap(const A& a) {
            return std::make_tuple(
                a ? std::optional<typename A::value_type>(a.unwrap()) : std::nullopt);
        }

        template <
            typename A, typename = std::enable_if_t<!std::is_array_v<A> && !is_result<A>::value>>
        std::tuple<std::optional<A>> __unwrap(const A& a) {
            return std::make_tuple(std::optional(a));
        }
        ...
        std::tuple<As...> __drop_one_impl(
            std::tuple<A, As...> tup, std::index_sequence<Is...> seq) {
        ...
        [[maybe_unused]] std::optional<std::tuple<>> __transpose() { return std::make_tuple(); }

        template <typename A>
        [[maybe_unused]] std::optional<std::tuple<A>> __transpose(
            std::tuple<std::optional<A>> tup) {
            auto elm = std::get<0>(tup);
            if (!elm.has_value()) return std::nullopt;

            return elm.value();
        }

        template <typename A, typename... As, typename = std::enable_if_t<sizeof...(As) != 0>>
        [[maybe_unused]] std::optional<std::tuple<A, As...>> __transpose(
            std::tuple<std::optional<A>, std::optional<As>...> tup) {
            auto elm = std::get<0>(tup);
            if (!elm.has_value()) return std::nullopt;
        ...
        template <typename A, typename = std::enable_if_t<!is_result<A>::value>>
        [[maybe_unused]] std::tuple<> __get_error(const A& a) {
            return std::tuple<>();
        }
    ...
    static ResultTy makeAST(Args&&... args) {
        ...
        if (opt.has_value()) {
            auto foozle = [](auto... xs) -> Ast* { return new Ast(xs...); };

            return Ok<Ast*>(std::apply(foozle, opt.value()));
        }

        return Err(std::apply(
            [](auto&&... xs) { return __concat_errors(xs...); },
            __get_error(std::forward<Args&&>(args)...)));
    }
    ...
    struct State {
        ...
        bool match(TT t) {
            if (tokens.size() == 0 || tokens.front() != t) return false;

            this->pop();
            return true;
        }

        const Token& peek(size_t n = 0) { return tokens.size() <= n ? eof : tokens[n]; }
    ...
    ResultTy parse(zbuf::str_view str) {
        if (str.empty()) return make_error({}, "empty input");
        ...
        if (!st.empty())
            return make_error(
                st.peek().loc, zpr::sprint("junk at end of expression: '{}'", st.peek().text));
    ...
    static ResultTy parseUnary(State& st) {
        ...
        return make_error(
            {}, st.empty() ? zpr::sprint("unexpected end of input")
    ...
    static ResultTy parseLambda(State& st) {
        ...
        if (st.peek() == TT::RightArrow || st.peek() == TT::Period) {
            ...
            return makeAST<ast::Lambda>(
                Location{l.begin, (*body)->loc.begin + (*body)->loc.length - l.begin}, arg.loc,
                arg.text.str(), body);
        } else if (st.peek() == TT::Identifier) {
            ...
            return makeAST<ast::Lambda>(
                Location{l.begin, (*sub)->loc.begin + (*sub)->loc.length - l.begin}, arg.loc,
                arg.text.str(), sub);
        } else {
            auto x = st.peek();
            return make_error(
                x.loc, zpr::sprint("expected '.' or '->' or identifier; found '{}'", x.text));
        }
    }

    static ResultTy parseApply(State& st, ResultTy lhs) {
        if (!lhs || st.empty()) return lhs;

        while (true) {
            // if it's time, then stop.
            if (st.peek() == TT::RParen || st.peek() == TT::EndOfFile) return lhs;
    ...
    static ResultTy parseLet(State& st) {
        ...
        if (name != TT::Identifier)
            return make_error(
                name.loc, zpr::sprint("expected identifier for 'let', found '{}'", name.text));
