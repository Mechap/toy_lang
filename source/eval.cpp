...
namespace lc {
    ...
    static void print_trace(int print_flags, const char* fmt, Args&&... args) {
        if (print_flags & FLAG_TRACE) zpr::println(fmt, static_cast<Args&&>(args)...);
    }
    ...
    Expr* evaluate(Context& ctx, const Expr* expr, int print_flags) {
        ...
        if (auto let = dynamic_cast<const Let*>(expr)) {
            ...
            print_trace(
                print_flags, "{}*.{} {}{}defined:{} {}{}{}", BLACK_BOLD, COLOUR_RESET, BLUE_BOLD,
        ...
        print_trace(
            print_flags, "{}0.{} {}", BLACK_BOLD, COLOUR_RESET, lc::print(copy, print_flags));
        ...
        print_trace(
            print_flags, "{}*.{} {}done.{}", BLACK_BOLD, COLOUR_RESET, BLUE_BOLD, COLOUR_RESET);
        return copy;
    }
        ...
        if (auto func = dynamic_cast<Lambda*>(app->fn); func != nullptr) {
            ...
            for (auto v : free) {
                ...
                if (auto it = bound.find(f); it != bound.end()) {
                    print_trace(
                        print_flags, "{}{}.{} {}α-con:{} {}{}{} <- {}", BLACK_BOLD, step++, 
                        OLOUR_RESET, GREEN, COLOUR_RESET, BLACK_BOLD, f, COLOUR_RESET,
                        fresh_name(f));

                    do_transform(
                        print_flags, [&]() { alpha_conversion(it->second, f, fresh_name(f)); },
                        logAlphaConversion, const_cast<const Expr**>(whole), it->second,
                        print_flags);
                }
            }
            ...
            print_trace(
                print_flags, "{}{}.{} {}β-red:{} {}{}{} <- {}", BLACK_BOLD, step++, COLOUR_RESET,
                YELLOW, COLOUR_RESET, BLACK_BOLD, func->arg, COLOUR_RESET,
                lc::print(app->arg, print_flags));
            ...
            do_transform(
                ...
                logBetaReduction, const_cast<const Expr**>(whole), func, app->arg, substs,
                print_flags);
...
namespace ast {
    ...
    Var* Var::clone() const { return new Var(this->loc, this->name); }
    ...
    Let* Let::clone() const { return new Let(this->loc, this->name, this->value->clone()); }
}  // namespace ast
