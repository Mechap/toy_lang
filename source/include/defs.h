...
namespace lc {
    ...
    std::pair<std::string, std::string> highlight(
        const ast::Expr* expr, std::function<std::optional<std::string>(const ast::Expr*)> pred,
    ...
    std::pair<std::string, std::string> logBetaReduction(
        const ast::Expr** whole, const ast::Expr* fn, const ast::Expr* arg,
        const std::vector<ast::Expr**>& substs, int print_flags);

    std::pair<std::string, std::string> logAlphaConversion(
        const ast::Expr** whole, const ast::Expr* sub, int print_flags);
