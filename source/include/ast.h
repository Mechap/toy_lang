...
namespace ast {
    ...
    struct Lambda : Expr {
        Lambda(parser::Location loc, parser::Location argloc, std::string arg, Expr* body)
            : Expr(TYPE, loc), argloc(argloc), arg(std::move(arg)), body(body) {}
    ...
    struct Let : Expr {
        Let(parser::Location loc, std::string name, Expr* value)
            : Expr(TYPE, loc), name(std::move(name)), value(value) {}
